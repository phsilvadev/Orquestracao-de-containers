...
class AuthController extends Controller
{
    ...
    public function me()
    {
        return auth()->user()->makeHidden(['password']);
    }
    ...
    public function refresh(){
        ...
        try {
            // Obtenha o novo token a partir do token atual
            $newToken = JWTAuth::refresh(JWTAuth::getToken());

            // Retorne o novo token para o cliente
            return response()->json([
                'success' => true,
                'token' => $newToken
            ]);
        } catch (\Tymon\JWTAuth\Exceptions\JWTException $e) {
            // Caso o refresh token não seja válido ou haja algum outro erro
            return response()->json([
                'success' => false,
